...
public class NPCControl_CHall : MonoBehaviour
{
    [Header("General Info")]
    ...
    private void StartDialog()
    {
        ...
        UI_CHall_Controller.instance.SetInfoDialog(DialogContent.NPCAva, DialogContent.NPCname);

        UI_CHall_Controller.instance.ShowNextButton(false);

        UI_CHall_Controller.instance.ShowDialogPanel(true);
    ...
    IEnumerator TypingContent()
    {
        ...
        {
            UI_CHall_Controller.instance.ShowNextButton(!SentenceCanBeAutoPass[CurrentDialogIndex]);
        }

        foreach (char letter in InsideCurrentDialog[CurrentDialogIndex])
        {
            string txt = UI_CHall_Controller.instance.GetDialogText();
            txt += letter;
            UI_CHall_Controller.instance.SetDialogText(txt);
    ...
    private void NextLine()
    {
        if (istyping) // đang gõ thì nhấn vào nó sẽ hết gõ 
        {
            StopAllCoroutines();
            istyping = false;
            UI_CHall_Controller.instance.SetDialogText(InsideCurrentDialog[CurrentDialogIndex]);
        }
        else
        {
    ...
    public void GiveReward(int id)
    {
        ...
        {
            // Show UI
            UI_CHall_Controller.instance.ShowReceiveObjectPanel(true);
            UI_CHall_Controller.instance.SetReceiveObject("Bạn nhận được " + obj.NameObject);
